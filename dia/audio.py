...
def codebook_to_audio(generated_codes: torch.Tensor, model, delay_pattern, B=1, T=2600, C=9):
    ...
    codebook = reverted_codebook.transpose(1, 2)  # [1, C, T]
    ...
    if total_codebooks == 9:
        # Mono decoding
        return decode(model, codebook)
    elif total_codebooks == 18:
        # Stereo decoding: split into two 9-codebook groups and decode separately
        left_codes = codebook[:, :9, :]
        right_codes = codebook[:, 9:, :]
        left_audio = decode(model, left_codes).squeeze()
        right_audio = decode(model, right_codes).squeeze()
        return torch.stack([left_audio, right_audio], dim=0)
    else:
        raise ValueError(f"Unsupported number of codebooks {total_codebooks}; expected 9 or 18.")