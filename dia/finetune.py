...
from .dataset import MusicDataset, LocalDiaDataset, HFDiaDataset, HFDiaIterDataset
...
test_prompts = {
    "rock_chill": "rock, chill, fun",
    "electronic_upbeat": "electronic, upbeat, energetic",
    "jazz_smooth": "jazz, smooth, relaxing",
    "classical_peaceful": "classical, peaceful, elegant",
    "ambient_dreamy": "ambient, dreamy, atmospheric"
}
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset",   type=str,  default="Paradoxia/opendata-iisys-hui",
                        help="HuggingFace dataset name (if not using --csv_path or --audio_folder).")
    ...
    parser.add_argument("--unconditional_frac", type=float, default=0.15,
        ...
        with torch.inference_mode():
            for test_name, prompt in test_prompts.items():
                try:
                    ...
                    writer.add_audio(f"Eval/{test_name}", audio, global_step, 44100)
                except:
                     logger.exception(f"Error synthesizing test prompt for {test_name}.")
                del audio
        gc.collect()
...
def main():
    ...
    if not dataset:
        if args.audio_folder:
            # Use new MusicDataset for audio folder structure
            dataset = MusicDataset(args.audio_folder, dia_cfg, dac_model)
        elif args.csv_path:
