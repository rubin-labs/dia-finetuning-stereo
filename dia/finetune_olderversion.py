...
from .text_augmentation import augment_text_string
# Dataset classes removed - use train_from_scratch.py for music training
# Removed speech dataset imports - using music datasets instead
...
# MUSIC2BYTE system removed - now using natural language conditioning

test_prompts = {
    "trap": "trap, heavy bass, hi-hats, snares, dark, aggressive",
    "hip-hop": "hip-hop, rhythmic beats, vocal samples, urban, groovy", 
    "rnb": "rnb, smooth, jazzy chords, soulful, melodic",
    "pop": "pop, catchy melody, upbeat tempo, commercial, bright",
    "electronic": "electronic, synthesized, digital effects, ambient, futuristic"
}
...
class TrainConfig:
    ...
    learning_rate: float = 1e-4     # SOTA: slightly higher LR
    warmup_steps: int = 4000        # SOTA: longer warmup
    label_smoothing: float = 0.1    # SOTA: label smoothing for stability
    unconditional_frac: float = 0.15
    tag_shuffle: bool = True      # Shuffle order of comma-separated tags
    tag_dropout: float = 0.15     # Probability of dropping individual tags
    eval_step: int = 200
    save_step: int = 2000
...
def collate_fn(batch, config: DiaConfig, device: torch.device, train_cfg: 'TrainConfig' = None):
    ...
    text_ids = []
    for txt in texts:
        # Apply tag augmentation during training
        if train_cfg is not None:
            txt = augment_text_string(
                txt, 
                shuffle=train_cfg.tag_shuffle, 
                dropout=train_cfg.tag_dropout
            )
        
        # Convert to UTF-8 bytes (no bracket replacement)
...
def setup_loaders(dataset, dia_cfg: DiaConfig, train_cfg: TrainConfig, device):
    collate = lambda b: collate_fn(b, dia_cfg, device, train_cfg)
    # HuggingFace streaming dataset support removed - use train_from_scratch.py for music training
    ds_len = len(dataset)
...
def setup_optimizer_and_scheduler(model, train_loader, train_cfg):
    ...
    # Fix for infinite epochs: use reasonable total training steps
    if train_cfg.epochs == float('inf') or train_cfg.epochs > 10000:
        # Use 50k steps as default for long training runs
...
def eval_step(model, val_loader, dia_cfg, dac_model, writer, global_step):
    ...
    try:
        ...
        with torch.inference_mode():
            for genre, prompt in test_prompts.items():
                try:
                    audio = dia_gen.generate(text=prompt)
                    writer.add_audio(f"Eval/{genre}", audio, global_step, 44100)
                except:
                     logger.exception(f"Error generating music for {genre}.")
                del audio
        gc.collect()
...
def main():
    ...
    # Speech datasets removed - use music datasets or train_from_scratch.py for music

    # choose dataset
    if not dataset:
        # All dataset loading removed - use train_from_scratch.py for music training
        raise ValueError("Use train_from_scratch.py for music generation training. This script is deprecated.")
